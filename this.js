...
// foo.call({name: 1});//или то что передал в call то и будет this
...
//
// const bindFoo = foo.bind({name: 3});//Функцию сразу не вызывает, возвращает обертку,
// // что бы запустить функцию, нужно вызвать обертку
// bindFoo();
// //способ без записи в переменную, просто в конце добавляют вызов
// foo.bind({name: 4})();//() - добавленный в конце вызов функции
// //вызов с аргументами
// foo.bind({name: 5}, 2, 3)();//в сам bind
// foo.bind({name: 6})(3, 4);//в вызов
// foo.bind({name: 7}, 5)(6);//или так
// const bindFoo1 = foo.bind({name: 8}, 1, 2);//Функцию сразу не вызывает, возвращает обертку,
...
//
// const result = new (UserConstructor.bind({name: 'Vita'}))();
// console.log(result);
// // this Будет UserConstructor {} - так как new главнее и выполнится он
// ---------------------------------------------------------------
...
// console.log(a.getMsg2());// this ---> window так как getMsg() стрелочная fn, а у обьекта 'a' нет своего LE
// -------------------------------------------------------------
// const a = {
//     name: 'a',
//     getName() {
...
//     cb();
// }
// foo(user.showName);//this будет window.name - потому что (user.snowName)это не вызов функции
// // вызов получается на строчке -> cb(), простой вызов а значит this window.name = ничего
// --------------------------------------------------------------
// const user ={
//     name: 'Vita',
//     showName(){
...
//     }
// }
// setTimeout(user.showName,1000);//потеря контекста, будет this window так как запись user.showName - означает
// // что сначала выполниться user.showName - вернет результат, а результат уэе сетТаймаутом вернется через 1 сек
